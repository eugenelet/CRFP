...
class generator(nn.Module):
    def __init__(self, spynet_pretrained, device):
        super().__init__()
        self.generator = BasicVSRNet(spynet_pretrained='pretrained_models/spynet_20210409-c6c1bd09.pth', device=device).to(device)
...
class MRCF_simple_v0(nn.Module):
    ...
    def forward(self, lrs, fvs, mks, fgs):
        ...
        for i in range(0, t):
            ...
            if torch.is_tensor(self.feat_prop_3):
...
class MRCF_simple_v13(nn.Module):
    ...
    def forward(self, lrs, fvs, mks, fgs):
        ...
        B, N, C, H, W = fgs.size()
        fg_lv3 = fgs.float()
        fg_lv0 = self.img_downsample_4x(fg_lv3.view(B*N, 1, H, W)).view(B, N, 1, H//4, W//4)
        ...
        for i in range(0, t):
            ...
            if torch.is_tensor(self.feat_prop_lv3):
                flow = flows_forward[:, i, :, :, :]
    ...
    def forward(self, lrs, fvs, mks, fgs):
        ...
        for i in range(0, t):
            ...
            if torch.is_tensor(self.feat_prop_lv3):
                ...
                feat_prop_lv3 = self.feat_prop_lv3
                feat_lv0 = self.feat_lv0
                feat_lv1 = self.feat_lv1
                feat_lv2 = self.feat_lv2
                ...
                feat_prop_lv0 = feat_prop_lv0 * fg_lv0_cur
                feat_prop_lv0_ = self.forward_resblocks_0(feat_prop_lv0_)
                ...
                feat_prop_lv0_ = feat_prop_lv0_ * fg_lv0_cur
                feat_prop_lv0_ = self.forward_resblocks_1(feat_prop_lv0_)
                feat_prop_lv0_ = torch.chunk(feat_prop_lv0_, 4, dim=1) # 5 -> 4
                ...
                feat_prop_lv3 = torch.cat([feat_prop_lv0_, feat_prop_lv3_a], dim=1)
                feat_prop_lv3 = feat_prop_lv3 * fg_lv3_cur
                feat_prop_lv3 = self.forward_resblocks_3(feat_prop_lv3)
            ...
            self.feat_prop_lv3 = feat_prop_lv3
            self.feat_lv0 = feat_lv0
            self.feat_lv1 = feat_lv1
            self.feat_lv2 = feat_lv2
...
class MRCF_simple_v18_cra(nn.Module):
    def __init__(self, device, mid_channels=16, y_only=False, hr_dcn=True, offset_prop=True, split_ratio=3, spynet_pretrained=None):
...
class MRCF_simple_v1_dcn2_v4_kai(nn.Module):
    ...
    def __init__(self, device, mid_channels=16, y_only=False, spynet_pretrained=None):
        ...
        self.feat_lv2 = None
        self.feat_lv3 = None
        self.feat_prop_lv0 = None
        self.feat_prop_lv1 = None
        self.feat_prop_lv2 = None
        self.feat_prop_lv3 = None
        ...
        # self.pca = PCD_Align(self.mid_channels, self.dg_num, self.dk, self.max_residue_magnitude)
        # self.dcn_0 = DCN_module(mid_channels, self.dg_num, self.dk, self.max_residue_magnitude)
        # self.dcn_1 = DCN_module(mid_channels, self.dg_num, self.dk, self.max_residue_magnitude)
        # self.dcn_2 = DCN_module(mid_channels, self.dg_num, self.dk, self.max_residue_magnitude)
        # self.dcn_3 = DCN_module(self.last_channels, 1,  self.dk, self.max_residue_magnitude)
        # self.dcn_1 = DCN_module(mid_channels, self.dg_num, self.dk, self.max_residue_magnitude, pre_offset=True, interpolate='none')
        # self.dcn_2 = DCN_module(mid_channels, self.dg_num, self.dk, self.max_residue_magnitude, pre_offset=True, interpolate='none')
        ...
        self.conv_tttf = conv3x3(self.last_channels * 2, self.last_channels)
        # self.conv_tttf_0 = conv3x3(mid_channels * 2, mid_channels)
        # self.conv_tttf_1 = conv3x3(mid_channels * 2, mid_channels)
        # self.conv_tttf_2 = conv3x3(mid_channels * 2, mid_channels)
        # self.conv_tttf_3 = conv3x3(mid_channels * 2, mid_channels)
        ...
        #                   nn.Conv2d(self.last_channels, mid_channels, 3, 1, 1),
        #                   nn.Upsample(scale_factor=0.25, mode='bilinear', align_corners=False))
        # self.downsample_0 = nn.Conv2d(self.last_channels, mid_channels, 3, 1, 1)
        # self.downsample_1 = nn.Conv2d(self.last_channels, mid_channels, 3, 1, 1)
        # self.downsample_2 = nn.Conv2d(self.last_channels, mid_channels, 3, 1, 1)
        # self.downsample_3 = nn.Conv2d(self.last_channels, mid_channels, 3, 1, 1)

        # upsample
        # self.upsample = PixelShufflePack(
        #     mid_channels, mid_channels * 3 // 4, 2, upsample_kernel=3)
        # self.upsample_post = PixelShufflePack(
        #     mid_channels * 3 // 4, self.last_channels, 4, upsample_kernel=3)
        self.upsample = PixelShufflePack(
            mid_channels, mid_channels, 2, upsample_kernel=3)
        self.upsample_post = PixelShufflePack(
            mid_channels, self.last_channels, 4, upsample_kernel=3)
    ...
    def forward(self, lrs, fvs, mks, fgs):
        ...
        feat_prop_lv3 = lrs.new_zeros(n, self.mid_channels, h*2, w*2)
        feat_prop_lv3_0 = lrs.new_zeros(n, self.last_channels, h*8, w*8)
        # feat_prop_lv0 = lrs.new_zeros(n, self.mid_channels, h*2, w*2)
        # feat_prop_lv1 = lrs.new_zeros(n, self.mid_channels, h*2, w*2)
        # feat_prop_lv2 = lrs.new_zeros(n, self.mid_channels, h*2, w*2)
        # feat_prop_lv3 = lrs.new_zeros(n, self.mid_channels, h*2, w*2)
        feat_lv0 = lrs.new_zeros(n, self.mid_channels//4, h*2, w*2)
        feat_lv1 = lrs.new_zeros(n, self.mid_channels//4, h*2, w*2)
        feat_lv2 = lrs.new_zeros(n, self.mid_channels//4, h*2, w*2)
        # feat_lv3 = lrs.new_zeros(n, self.mid_channels//4, h*8, w*8)
        ...
        for i in range(0, t):
            ...
            # mk_lv0_cur = self.img_downsample_4x(mk_cur.float())
            # feat_prop_cur = self.upsample(x_lr_lv0_cur)
            ...
            if torch.is_tensor(self.feat_prop_lv3):
                ...
                # feat_prop_lv0 = self.feat_prop_lv0
                # feat_prop_lv1 = self.feat_prop_lv1
                # feat_prop_lv2 = self.feat_prop_lv2
                feat_prop_lv3 = self.feat_prop_lv3
                feat_lv0 = self.feat_lv0
                feat_lv1 = self.feat_lv1
                feat_lv2 = self.feat_lv2
                # feat_lv3 = self.feat_lv3
                ...
                # feat_mix = flow_warp(feat_mix, flow_lv3.permute(0, 2, 3, 1))
                # # feat_lv3 = flow_warp(feat_lv3, flow_lv0.permute(0, 2, 3, 1))
                ...
                # feat_prop_lv0_ = torch.cat([feat_prop_lv0, feat_prop_lv3], dim=1)
                # feat_prop_lv0_ = self.conv_tttf_0(feat_prop_lv0_)
                # feat_prop_lv0 = mk_lv0_cur.float() * feat_prop_lv0_ + (1 - mk_lv0_cur.float()) * feat_prop_lv0
                # feat_prop_lv1_ = torch.cat([feat_prop_lv1, feat_prop_lv3], dim=1)
                # feat_prop_lv1_ = self.conv_tttf_1(feat_prop_lv1_)
                # feat_prop_lv1 = mk_lv0_cur.float() * feat_prop_lv1_ + (1 - mk_lv0_cur.float()) * feat_prop_lv1
                # feat_prop_lv2_ = torch.cat([feat_prop_lv2, feat_prop_lv3], dim=1)
                # feat_prop_lv2_ = self.conv_tttf_2(feat_prop_lv2_)
                # feat_prop_lv2 = mk_lv0_cur.float() * feat_prop_lv2_ + (1 - mk_lv0_cur.float()) * feat_prop_lv2

                # feat_prop_mix = torch.cat((feat_prop_lv0,
                #                            feat_prop_lv1,
                #                            feat_prop_lv2,
                #                            feat_prop_lv3), dim=1)
                ...
                # feat_prop_mix = torch.chunk(feat_prop_mix, 4, dim=1)
                # feat_prop_lv0_ = feat_prop_mix[0]
                # feat_prop_lv1_ = feat_prop_mix[1]
                # feat_prop_lv2_ = feat_prop_mix[2]
                # feat_prop_lv3_ = feat_prop_mix[3]

                # feat_prop_lv3 = self.pca(feat_prop_lv0, feat_prop_lv3, feat_prop_lv3_, flow_lv3)
                ...
                # feat_prop_lv0_zeros = torch.cat([feat_prop_lv0, feat_prop_lv0.new_zeros(n, self.mid_channels*2, h*2, w*2)], dim=1)
                # feat_prop_lv0_ = torch.cat([feat_prop_lv0, feat_prop_lv3_a, feat_prop_lv3_], dim=1)
                # feat_prop_lv0_ = fg_lv0_cur * feat_prop_lv0_ + (1 - fg_lv0_cur) * feat_prop_lv0_zeros
                # feat_prop_lv0_ = self.forward_resblocks_0(feat_prop_lv0_)
                
                # # L1
                # # feat_prop_lv3_a, offset = self.dcn_1(feat_prop_lv0_, feat_prop_lv3, feat_prop_lv3_, flow_lv3)
                # feat_prop_lv3_a, offset = self.dcn_1(feat_prop_lv0_, feat_prop_lv3, feat_prop_lv3_, flow_lv3, offset)
                
                # feat_prop_lv0_zeros = torch.cat([feat_prop_lv0_, feat_prop_lv0.new_zeros(n, self.mid_channels*2, h*2, w*2)], dim=1)
                # feat_prop_lv0_ = torch.cat([feat_prop_lv0_, feat_prop_lv3_a, feat_prop_lv3_], dim=1)
                # feat_prop_lv0_ = fg_lv0_cur * feat_prop_lv0_ + (1 - fg_lv0_cur) * feat_prop_lv0_zeros
                # feat_prop_lv0_ = self.forward_resblocks_1(feat_prop_lv0_)

                # # L2
                # # feat_prop_lv3_a, offset = self.dcn_2(feat_prop_lv0_, feat_prop_lv3, feat_prop_lv3_, flow_lv3)
                # feat_prop_lv3_a, offset = self.dcn_2(feat_prop_lv0_, feat_prop_lv3, feat_prop_lv3_, flow_lv3, offset)
                ...
                # # L3
                # feat_prop_lv0_ = self.lrelu(self.upsample_post(feat_prop_lv0_))
                # # feat_prop_lv3_a, _ = self.dcn_3(feat_prop_lv0_, feat_prop_lv3_0, feat_prop_lv3_0_, flow_lv0)
                # feat_prop_lv3_a, _ = self.dcn_3(feat_prop_lv0_, feat_prop_lv3_0, feat_prop_lv3_0_, flow_lv0, offset)
                # # feat_prop_lv3_a, _ = self.dcn_3(feat_prop_lv0_, feat_prop_lv3, feat_prop_lv3_, flow_lv3, offset)
                
                # feat_prop_lv0_zeros = torch.cat([feat_prop_lv0_, feat_prop_lv0.new_zeros(n, self.last_channels*2, h*8, w*8)], dim=1)
                # feat_prop_lv3 = torch.cat([feat_prop_lv0_, feat_prop_lv3_a, feat_prop_lv3_0_], dim=1)
                # feat_prop_lv3 = fg_lv3_cur * feat_prop_lv3 + (1 - fg_lv3_cur) * feat_prop_lv0_zeros
                # # feat_prop_lv3 = torch.cat([feat_prop_lv0_, feat_prop_lv3_a, feat_prop_lv3_], dim=1)
                # feat_prop_lv3 = self.forward_resblocks_3(feat_prop_lv3)
                
                #### v16
                # # L0             
                # feat_prop_lv0, _ = self.dcn_0(feat_prop_cur, feat_prop_lv0, feat_prop_lv0_, flow_lv3)

                # # feat_prop_lv0 = torch.cat([feat_prop_cur, feat_prop_lv0_, feat_prop_lv0], dim=1)
                # feat_prop_lv0 = torch.cat([feat_prop_cur, feat_prop_lv0], dim=1)
                # feat_prop_lv0 = self.forward_resblocks_0(feat_prop_lv0)
                # # x_hr_lv0_cur = self.lrelu(self.img_downsample_4x(self.downsample_0(x_hr_lv3_cur)))
                # # feat_prop_lv0_ = torch.cat([feat_prop_lv0, x_hr_lv0_cur], dim=1)
                # # feat_prop_lv0_ = torch.cat([feat_prop_lv0, feat_prop_lv3_0], dim=1)
                # # feat_prop_lv0_ = self.conv_tttf_0(feat_prop_lv0_)
                # # feat_prop_lv0 = mk_lv0_cur.float() * feat_prop_lv0_ + (1 - mk_lv0_cur.float()) * feat_prop_lv0
                
                # # L1
                # feat_prop_lv1, _ = self.dcn_1(feat_prop_lv0, feat_prop_lv1, feat_prop_lv1_, flow_lv3)
                
                # # feat_prop_lv1 = torch.cat([feat_prop_lv0, feat_prop_lv1_, feat_prop_lv1], dim=1)
                # feat_prop_lv1 = torch.cat([feat_prop_lv0, feat_prop_lv1], dim=1)
                # feat_prop_lv1 = self.forward_resblocks_1(feat_prop_lv1)
                # # x_hr_lv0_cur = self.lrelu(self.img_downsample_4x(self.downsample_1(x_hr_lv3_cur)))
                # # feat_prop_lv1_ = torch.cat([feat_prop_lv1, x_hr_lv0_cur], dim=1)
                # # feat_prop_lv1_ = self.conv_tttf_1(feat_prop_lv1_)
                # # feat_prop_lv1 = mk_lv0_cur.float() * feat_prop_lv1_ + (1 - mk_lv0_cur.float()) * feat_prop_lv1

                # # L2
                # feat_prop_lv2, _ = self.dcn_2(feat_prop_lv1, feat_prop_lv2, feat_prop_lv2_, flow_lv3)
                
                # # feat_prop_lv2 = torch.cat([feat_prop_lv1, feat_prop_lv2_, feat_prop_lv2], dim=1)
                # feat_prop_lv2 = torch.cat([feat_prop_lv1, feat_prop_lv2], dim=1)
                # feat_prop_lv2 = self.forward_resblocks_2(feat_prop_lv2)
                # # x_hr_lv0_cur = self.lrelu(self.img_downsample_4x(self.downsample_2(x_hr_lv3_cur)))
                # # feat_prop_lv2_ = torch.cat([feat_prop_lv2, x_hr_lv0_cur], dim=1)
                # # feat_prop_lv2_ = self.conv_tttf_2(feat_prop_lv2_)
                # # feat_prop_lv2 = mk_lv0_cur.float() * feat_prop_lv2_ + (1 - mk_lv0_cur.float()) * feat_prop_lv2

                # # L3
                # feat_prop_lv3, _ = self.dcn_3(feat_prop_lv2, feat_prop_lv3, feat_prop_lv3_, flow_lv3)
                
                # # feat_prop_lv3 = torch.cat([feat_prop_lv2, feat_prop_lv3_, feat_prop_lv3], dim=1)
                # feat_prop_lv3 = torch.cat([feat_prop_lv2, feat_prop_lv3], dim=1)
                # feat_prop_lv3 = self.forward_resblocks_3(feat_prop_lv3)
                # # x_hr_lv0_cur = self.lrelu(self.img_downsample_4x(self.downsample_3(x_hr_lv3_cur)))
                # # feat_prop_lv3_ = torch.cat([feat_prop_lv3, x_hr_lv0_cur], dim=1)
                # # feat_prop_lv3_ = self.conv_tttf_3(feat_prop_lv3_)
                # # feat_prop_lv3 = mk_lv0_cur.float() * feat_prop_lv3_ + (1 - mk_lv0_cur.float()) * feat_prop_lv3
                ...
                # feat_prop_lv0_ = torch.cat([feat_prop_lv0, feat_prop_lv3_a], dim=1)
                # feat_prop_lv0_ = self.forward_resblocks_0(feat_prop_lv0_)
                # # feat_prop_lv0_ = torch.chunk(feat_prop_lv0_, 5, dim=1)
                # # feat_lv0 = feat_prop_lv0_[4]
                # # feat_prop_lv0_ = torch.cat(feat_prop_lv0_[:4], dim=1)
                # feat_prop_lv0_ = torch.chunk(feat_prop_lv0_, 4, dim=1)
                # feat_lv0 = feat_prop_lv0_[3]
                # feat_prop_lv0_ = torch.cat(feat_prop_lv0_[:3], dim=1)
                ...
                #### v16
                # # L0                
                # # feat_prop_lv0 = torch.cat([feat_prop_cur, feat_prop_lv0, feat_prop_lv0], dim=1)
                # feat_prop_lv0 = torch.cat([feat_prop_cur, feat_prop_lv0], dim=1)
                # feat_prop_lv0 = self.forward_resblocks_0(feat_prop_lv0)
                # # x_hr_lv0_cur = self.lrelu(self.img_downsample_4x(self.downsample_0(x_hr_lv3_cur)))
                # # feat_prop_lv0_ = torch.cat([feat_prop_lv0, x_hr_lv0_cur], dim=1)
                ...
                # # L1
                # # feat_prop_lv1 = torch.cat([feat_prop_lv0, feat_prop_lv1, feat_prop_lv1], dim=1)
                # feat_prop_lv1 = torch.cat([feat_prop_lv0, feat_prop_lv1], dim=1)
                # feat_prop_lv1 = self.forward_resblocks_1(feat_prop_lv1)
                # # x_hr_lv0_cur = self.lrelu(self.img_downsample_4x(self.downsample_1(x_hr_lv3_cur)))
                # # feat_prop_lv1_ = torch.cat([feat_prop_lv1, x_hr_lv0_cur], dim=1)
                ...
                # # L2
                # # feat_prop_lv2 = torch.cat([feat_prop_lv1, feat_prop_lv2, feat_prop_lv2], dim=1)
                # feat_prop_lv2 = torch.cat([feat_prop_lv1, feat_prop_lv2], dim=1)
                # feat_prop_lv2 = self.forward_resblocks_2(feat_prop_lv2)
                # # x_hr_lv0_cur = self.lrelu(self.img_downsample_4x(self.downsample_2(x_hr_lv3_cur)))
                # # feat_prop_lv2_ = torch.cat([feat_prop_lv2, x_hr_lv0_cur], dim=1)
                ...
                # # L3
                # # feat_prop_lv3 = torch.cat([feat_prop_lv2, feat_prop_lv3, feat_prop_lv3], dim=1)
                # feat_prop_lv3 = torch.cat([feat_prop_lv2, feat_prop_lv3], dim=1)
                # feat_prop_lv3 = self.forward_resblocks_3(feat_prop_lv3)
                # # x_hr_lv0_cur = self.lrelu(self.img_downsample_4x(self.downsample_3(x_hr_lv3_cur)))
                # # feat_prop_lv3_ = torch.cat([feat_prop_lv3, x_hr_lv0_cur], dim=1)
            ...
            # out = self.lrelu(self.upsample_post(feat_prop_lv3))
            
            # self.feat_prop_lv0 = feat_prop_lv0
            # self.feat_prop_lv1 = feat_prop_lv1
            # self.feat_prop_lv2 = feat_prop_lv2
            self.feat_prop_lv3 = feat_prop_lv3
            self.feat_lv0 = feat_lv0
            self.feat_lv1 = feat_lv1
            self.feat_lv2 = feat_lv2
            # self.feat_lv3 = feat_lv3
